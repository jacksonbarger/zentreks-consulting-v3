...
export default function Header() {
  ...
  return (
    <>
      {/* Promo Banner - Above the fixed header */}
      <div className="fixed top-0 left-0 right-0 z-[60]">
        <PromoBanner />
      </div>

      {/* Main Header - Positioned below the banner */}
      <header className="fixed top-[41px] left-0 right-0 z-50 bg-[#0D0D0D]/95 backdrop-blur-sm border-b border-[#2A2A2A]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            {/* Logo - Zentreks Official Logo */}
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {NAV_ITEMS.map((item) => (
                ...
                >
                  ...
                  {/* Mega Menu Dropdown - Updated colors */}
                  {item.children && (
                    <AnimatePresence>
                      {activeDropdown === item.label && (
                        <motion.div
                          ...
                          className="absolute top-full left-0 w-80 bg-[#141414] border border-[#2A2A2A] shadow-xl"
                        >
                          <div className="p-4">
                            {item.children.map((child) => (
                              <Link
                                key={child.label}
                                href={child.href}
                                className="block p-3 hover:bg-[#1E1E1E] transition-colors group"
                              >
                                <div className="flex items-center justify-between">
                                  <span className="text-white font-medium group-hover:text-[#D4A574] transition-colors">
                                    {child.label}
                                  </span>
                                  <ArrowRight size={14} className="text-[#8A7A7A] group-hover:text-[#D4A574] transition-colors" />
                                </div>
                                <p className="text-sm text-[#8A7A7A] mt-1">{child.description}</p>
                              </Link>
                            ))}
                          </div>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden bg-[#141414] border-t border-[#2A2A2A]"
            >
                      ...
                      <div className="pl-4 mt-2 space-y-2">
                        {item.children.map((child) => (
                          <Link
                            key={child.label}
                            href={child.href}
                            className="block text-[#8A7A7A] py-1 hover:text-[#D4A574] transition-colors"
                            onClick={() => setMobileMenuOpen(false)}
                          >
                            {child.label}
                          </Link>
